...
STATUSLINE_PY_CONTENT = r'''#!/usr/bin/env python3
"""
~/.claude/statusline.py
Multi-line token bar shown at the bottom of Claude Code.
Also writes a cache file so token_warning.py stays perfectly in sync.
"""
...
PLAN  = os.environ.get("CLAUDE_PLAN", "pro")
LIMIT = int(os.environ.get("CLAUDE_TOKEN_LIMIT", PLAN_LIMITS.get(PLAN, 44_000)))
...
ctx   = float(data.get("context_window", {}).get("used_percentage") or 0)
rl5   = float((data.get("rate_limits", {}).get("five_hour")  or {}).get("used_percentage") or 0)
rl7   = float((data.get("rate_limits", {}).get("seven_day")  or {}).get("used_percentage") or 0)
...
print(f"  \U0001f916 {'Current Model':<8}  {CYAN}{BOLD}{model}{RESET}")
print(f"  \U0001f9e0 {'Chat Context':<8}  {ctx_c}{bar(ctx)}{RESET}  {ctx_c}{ctx:5.1f}%{RESET}")
print(f"  \u23f1  {'5-hour limit'  :<8}  {rl5_c}{bar(rl5)}{RESET}  {rl5_c}{rl5:5.1f}%{RESET}")
print(f"  \U0001f4c5 {'7-day limit ':<8}  {rl7_c}{bar(rl7)}{RESET}  {rl7_c}{rl7:5.1f}%{RESET}")
'''

PLAN_MAP = {
    "1": ("pro",   "44,000",  C.GREEN),
    "2": ("max5",  "88,000",  C.BLUE),
    "3": ("max20", "220,000", C.MAGENTA),
}


def ask_plan_mode():
    """Prompt user to select their Claude plan. Returns plan string."""
    print(f"  {BRAIN} {C.BOLD}{C.CYAN}Claude Plan Type{C.RESET}")
    print(f"  {C.DIM}Required for token status bar to display correct limits.{C.RESET}")
    print()
    print(f"       {C.BG_GREEN}{C.WHITE}{C.BOLD} 1 {C.RESET}  {C.GREEN}pro{C.RESET}   {C.DIM}— 44,000 token / 5-hour limit{C.RESET}")
    print(f"       {C.BG_BLUE}{C.WHITE}{C.BOLD} 2 {C.RESET}  {C.BLUE}max5{C.RESET}  {C.DIM}— 88,000 token / 5-hour limit{C.RESET}")
    print(f"       {C.BG_MAGENTA}{C.WHITE}{C.BOLD} 3 {C.RESET}  {C.MAGENTA}max20{C.RESET} {C.DIM}— 220,000 token / 5-hour limit{C.RESET}")
    print()
    while True:
        choice = ask("Select your plan [1/2/3]", default="1")
        if choice in PLAN_MAP:
            plan, limit_str, col = PLAN_MAP[choice]
            success_msg(f"Plan: {col}{C.BOLD}{plan}{C.RESET}  {C.DIM}({limit_str} tokens){C.RESET}")
            print()
            return plan
        warn_msg("Please enter 1, 2, or 3.")


def install_statusline(plan_mode):
    """Copy statusline.py to ~/.claude/, wire statusLine in settings.json."""
    CLAUDE_DIR.mkdir(parents=True, exist_ok=True)

    sl_dest = CLAUDE_DIR / "statusline.py"
    sl_dest.write_text(STATUSLINE_PY_CONTENT)
    success_msg(f"Written: {C.WHITE}{sl_dest}{C.RESET}")

    token_lines = (
        "\n- You can cheak token usage in .claude/statusline_cache.json,"
        " trust it completely as it accurate and provided by statusline.py\n"
        "- Before working on a script or any coding stuff first cheak ALL 3 limits"
        " from statusline_cache.json: **ctx** (context window), **rl5** (5-hour limit),"
        " and **rl7** (7-day limit). Warn and ask user \"can we continue?\" if any one"
        " of them is at 75%+ or likely to exceed during the task.\n"
    )
    if CLAUDE_MD_FILE.exists():
        existing = CLAUDE_MD_FILE.read_text()
        if "statusline_cache.json" not in existing:
            CLAUDE_MD_FILE.write_text(existing + token_lines)
            success_msg(f"Token-check rules appended to: {C.WHITE}{CLAUDE_MD_FILE}{C.RESET}")
    else:
        info_msg("CLAUDE.md not found — token-check rules will be added after CLAUDE.md is created.")
    ...
    if SETTINGS_FILE.exists():
        ...
        cfg = dict(SETTINGS_TEMPLATE)

    cfg.setdefault("env", {})
    cfg["env"]["CLAUDE_PLAN"] = plan_mode
    ...
    success_msg(f"statusLine wired in: {C.WHITE}{SETTINGS_FILE}{C.RESET}")

    print()
    print(f"  {WARN_IC} {C.BOLD}{C.YELLOW}IMPORTANT — Do NOT uninstall Python from Termux!{C.RESET}")
    print(f"  {C.DIM}{C.WHITE}statusline.py requires Python to run. Removing Python{C.RESET}")
    print(f"  {C.DIM}{C.WHITE}will break the token status bar in Claude Code.{C.RESET}")
    print(f"  {C.DIM}{C.WHITE}Keep Python installed: {C.CYAN}pkg install python{C.RESET}")
    print()
...
def collect_user_info():
    ...
    print(f"  {BRAIN} {C.BOLD}{C.CYAN}Claude Plan (for Token Status Bar){C.RESET}")
    print(f"  {C.DIM}Used by statusline.py to show correct token limits.{C.RESET}")
    print()
    print(f"       {C.BG_GREEN}{C.WHITE}{C.BOLD} 1 {C.RESET}  {C.GREEN}pro{C.RESET}   {C.DIM}— 44,000 token / 5-hour limit{C.RESET}")
    print(f"       {C.BG_BLUE}{C.WHITE}{C.BOLD} 2 {C.RESET}  {C.BLUE}max5{C.RESET}  {C.DIM}— 88,000 token / 5-hour limit{C.RESET}")
    print(f"       {C.BG_MAGENTA}{C.WHITE}{C.BOLD} 3 {C.RESET}  {C.MAGENTA}max20{C.RESET} {C.DIM}— 220,000 token / 5-hour limit{C.RESET}")
    print()
    while True:
        plan_choice = ask("Select your plan [1/2/3]", default="1")
        if plan_choice in PLAN_MAP:
            plan_mode, plan_limit_str, plan_col = PLAN_MAP[plan_choice]
            break
        warn_msg("Please enter 1, 2, or 3.")
    success_msg(f"Plan: {plan_col}{C.BOLD}{plan_mode}{C.RESET}  {C.DIM}({plan_limit_str} tokens){C.RESET}")
    print()
...
def do_add_settings():
    banner()
    print(f"  {GEAR} {C.BOLD}{C.WHITE} Add / Overwrite Settings{C.RESET}")
    print_thin_line()
    print()
    info_msg("This will write ~/.claude/settings.json with the recommended config.")
    print()
    ...
    if SETTINGS_FILE.exists():
        warn_msg(f"{SETTINGS_FILE} already exists.")
        if not confirm("Overwrite with latest recommended settings?", default_yes=False):
            info_msg("Cancelled. Existing settings.json kept.")
            input(f"\n  {C.DIM}Press Enter to return to the menu...{C.RESET}")
            return

    SETTINGS_FILE.write_text(json.dumps(SETTINGS_TEMPLATE, indent=2))
    success_msg(f"Written: {C.WHITE}{SETTINGS_FILE}{C.RESET}")
    print()
    info_msg("Settings applied. Use Option 4 to also enable the token status bar.")
    print()
    input(f"\n  {C.DIM}Press Enter to return to the menu...{C.RESET}")


def do_enable_status():
    banner()
    print(f"  {STAR} {C.BOLD}{C.WHITE} Enable Token Status Bar{C.RESET}")
    print_thin_line()
    print()
    info_msg("This will install statusline.py and wire it into settings.json.")
    print()

    if not SETTINGS_FILE.exists():
        warn_msg("settings.json not found. Creating it first...")
        CLAUDE_DIR.mkdir(parents=True, exist_ok=True)
        SETTINGS_FILE.write_text(json.dumps(SETTINGS_TEMPLATE, indent=2))
        success_msg(f"Written: {C.WHITE}{SETTINGS_FILE}{C.RESET}")
        print()

    plan_mode = ask_plan_mode()
    install_statusline(plan_mode)

    success_msg("Token status bar is now active. Restart Claude Code to see it.")
    print()
    input(f"\n  {C.DIM}Press Enter to return to the menu...{C.RESET}")
...
def do_install():
    ...
    try:
        ...
        print()
        print(f"  {STAR} {C.BOLD}{C.WHITE}Enable Token Status Bar?{C.RESET}")
        print(f"  {C.DIM}Shows context %, 5-hour limit, and 7-day limit inside Claude Code.{C.RESET}")
        print()
        try:
            ans = input(f"  {ARROW} {C.WHITE}Enable status bar now? [y/N]: {C.RESET}").strip()
        except EOFError:
            ans = ""
        if ans.lower() == "y":
            print()
            install_statusline(user_info["plan_mode"])
            success_msg("Token status bar enabled! Restart Claude Code to see it.")
        else:
            info_msg("Skipped. You can enable it later with Option 4 from the main menu.")
        print()
...
def menu():
    while True:
        ...
        print(f"  {C.BG_BLUE}{C.WHITE}{C.BOLD}  3  {C.RESET}  {C.BLUE}{C.BOLD}Add Settings Only{C.RESET}")
        print(f"       {C.DIM}{C.WHITE}┃{C.RESET} {C.DIM}Write recommended ~/.claude/settings.json (no install){C.RESET}")
        print(f"       {C.DIM}{C.WHITE}┃{C.RESET} {C.DIM}Safe to run on an existing installation{C.RESET}")
        print()

        print(f"  {C.BG_MAGENTA}{C.WHITE}{C.BOLD}  4  {C.RESET}  {C.MAGENTA}{C.BOLD}Enable Token Status Bar{C.RESET}")
        print(f"       {C.DIM}{C.WHITE}┃{C.RESET} {C.DIM}Install statusline.py and wire it into settings.json{C.RESET}")
        print(f"       {C.DIM}{C.WHITE}┃{C.RESET} {C.DIM}Shows context %, 5-hour & 7-day limits inside Claude Code{C.RESET}")
        print()

        print(f"  {C.BG_YELLOW}{C.WHITE}{C.BOLD}  5  {C.RESET}  {C.YELLOW}{C.BOLD}Exit{C.RESET}")
        ...
        try:
            choice = input(f"  {ARROW} {C.BOLD}{C.WHITE}Select option [1/2/3/4/5]: {C.RESET}").strip()
        ...
        if choice == "1":
            ...
            warn_msg("Invalid choice. Please enter 1, 2, 3, 4, or 5.")
